...
import { getNewtonAuthHeaders, NewtonAuthInput } from "./auth.js";
...
const PLAYGROUND_KIND_FALLBACKS = ["coding", "react", "frontend", "project"] as const;

type PlaygroundKind = (typeof PLAYGROUND_KIND_FALLBACKS)[number] | string;

function normalizePlaygroundKind(value?: string): PlaygroundKind | undefined {
  ...
  if (normalized.includes("coding") || normalized.includes("dsa")) return "coding";

  const safeSlug = normalized.replace(/[^a-z0-9_-]/g, "");
  return safeSlug || undefined;
}

function uniquePlaygroundKinds(values: Array<PlaygroundKind | undefined>): PlaygroundKind[] {
  const seen = new Set<string>();
  const result: PlaygroundKind[] = [];
...
function buildPlaygroundKindCandidates(preferredKinds: Array<PlaygroundKind | undefined> = []): PlaygroundKind[] {
...
function isPlaygroundKindMiss(err: unknown): boolean {
...
function createClient(auth?: NewtonAuthInput): AxiosInstance {
...
function maybeJsonObject(value: string): Record<string, unknown> | undefined {
  const trimmed = value.trim();
...
function parseSubmissionRef(submissionId: string): {
  ...
  if (!playgroundRef) {
    throw new Error(
      "Invalid submissionId. Expected '<playgroundHash>' or '<playgroundHash>:<submissionHash>' (optionally '<kind>/<playgroundHash>:<submissionHash>')."
    );
  }

  let playgroundKind: PlaygroundKind | undefined;
...
function extractPlaygroundKindHints(source?: Record<string, unknown>): PlaygroundKind[] {
  if (!source) return [];
  ...
  return uniquePlaygroundKinds([
    normalizePlaygroundKind(
      firstString(
        source.playground_type,
        source.playground_kind,
    ...
    normalizePlaygroundKind(
      firstString(
        assignmentQuestion?.playground_type,
        assignmentQuestion?.playground_kind,
        assignmentQuestion?.question_type,
        assignmentQuestion?.type
      )
    ),
    normalizePlaygroundKind(
      firstString(
        isObject(question) ? question.playground_type : undefined,
        isObject(question) ? question.question_type : undefined,
        isObject(question) ? question.type : undefined
      )
    ),
  ]);
}
...
function normalizeSubmissionStatus(raw: Record<string, unknown>): SubmissionStatus {
  ...
  const allTestCasesPassed = maybeBoolean(
    raw.all_test_cases_passed ??
...
function buildPlaygroundPath(playgroundHash: string, playgroundKind: PlaygroundKind): string {
  return `/api/v1/playground/${encodeURIComponent(playgroundKind)}/h/${encodeURIComponent(playgroundHash)}/`;
}

function buildLatestSubmissionPath(
  playgroundHash: string,
  playgroundKind: PlaygroundKind
): string {
...
function buildSubmitPath(playgroundHash: string, playgroundKind: PlaygroundKind): string {
...
async function fetchPlaygroundByHash(
  ...
  playgroundHash: string,
  preferredKinds: PlaygroundKind[] = []
): Promise<{ playground: Record<string, unknown>; playgroundKind: PlaygroundKind }> {
  ...
  for (const playgroundKind of candidates) {
    try {
      const response = await withRetry(
        () => client.get(buildPlaygroundPath(playgroundHash, playgroundKind)),
        2,
        400,
        (err) => !isPlaygroundKindMiss(err)
      );
...
function extractSubjectHashFromAssignment(
  assignment?: Record<string, unknown>
): string | undefined {
  ...
  return firstString(
    assignment.subject_hash,
    assignment.course_hash,
    course?.subject_hash,
    course?.hash,
    subject?.hash
  );
...
async function findAssignmentByHash(
  client: AxiosInstance,
  courseHash: string,
  assignmentHash: string
  ...
  return assignments.find(
    (assignment) => firstString(assignment.hash, assignment.assignment_hash) === assignmentHash
  );
}
...
async function fetchAssignmentQuestionDetails(
  client: AxiosInstance,
  params: {
    courseHashCandidates: string[];
    assignmentHash: string;
    questionHash: string;
  }
): Promise<{ details: Record<string, unknown>; resolvedCourseHash: string }> {
...
async function resolvePlayground(
  ...
  params: {
    ...
    courseHash?: string;
    assignmentHash?: string;
    preferredPlaygroundKind?: PlaygroundKind;
  }
): Promise<{
  playgroundKind: PlaygroundKind;
  playgroundHash: string;
  ...
  try {
    ...
    const resolvedPlaygroundHash =
      firstString(fetched.playground.hash, requestedHash) ?? requestedHash;
    return {
  ...
  if (!assignmentHash) {
    assignment = await resolveAssignmentForQuestionHash(
      client,
      courseHashInput,
      requestedHash
    );
  ...
  if (!assignmentHash) {
    throw new Error(
      `Unable to find assignment for question hash '${requestedHash}' in course '${courseHashInput}'.`
    );
  }
...
function resolveLanguageId(
  ...
  if (!target && options?.allowMissing) return undefined;
  if (!target) throw new Error("language cannot be empty");
...
async function fetchLatestSubmissionRaw(
  ...
  playgroundHash: string,
  preferredKinds: PlaygroundKind[] = []
): Promise<{ latestSubmission: Record<string, unknown>; playgroundKind: PlaygroundKind }> {
  const candidates = buildPlaygroundKindCandidates(preferredKinds);
  let lastMiss: unknown;
  ...
  for (const playgroundKind of candidates) {
    try {
      const response = await withRetry(
        () => client.get(buildLatestSubmissionPath(playgroundHash, playgroundKind)),
        2,
        400,
        (err) => !isPlaygroundKindMiss(err)
      );
      ...
      if (playgroundKind === "project" && Object.keys(response.data).length === 0) {
        const playgroundResponse = await withRetry(
          () => client.get(buildPlaygroundPath(playgroundHash, playgroundKind)),
          2,
          400,
          (err) => !isPlaygroundKindMiss(err)
...
async function submitPlaygroundCode(
  client: AxiosInstance,
  playgroundHash: string,
  payload: Record<string, unknown>,
  preferredKinds: PlaygroundKind[] = []
): Promise<PlaygroundKind> {
  const candidates = buildPlaygroundKindCandidates(preferredKinds);
  let lastMiss: unknown;
  ...
  for (const playgroundKind of candidates) {
    try {
      await withRetry(
        () => client.patch(buildSubmitPath(playgroundHash, playgroundKind), payload),
        2,
        400,
        (err) => !isPlaygroundKindMiss(err)
      );
...
async function pollLatestSubmission(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    const fetched = await fetchLatestSubmissionRaw(
      client,
      playgroundHash,
...
export async function fetchProblem(
  problemId: string,
  options?: { courseHash?: string; assignmentHash?: string; auth?: NewtonAuthInput }
): Promise<Problem> {
  try {
...
export async function submitSolution(
  ...
  try {
    ...
    const submitKind = await submitPlaygroundCode(
      client,
      resolved.playgroundHash,
      payload,
      uniquePlaygroundKinds([
        preferredPlaygroundKind,
    ...
    return {
      submissionId: encodeSubmissionId(
        resolved.playgroundHash,
        polled.latestSubmission,
        polled.playgroundKind
      ),
...
export async function getSubmissionStatus(
  ...
  try {
    ...
    const latest = await fetchLatestSubmissionRaw(
      client,
      playgroundHash,
      playgroundKind ? [playgroundKind] : []
    );
