...
server.registerTool(
  ...
  {
    description:
      "Fetch a Newton coding problem. problemId can be playground hash, assignment-question hash, or course/assignment/question reference.",
  ...
  async ({ problemId, courseHash, assignmentHash }) => {
    try {
      return toolSuccess(
        await fetchProblem(problemId, { courseHash, assignmentHash })
      );
    } catch (err) {
      throw wrapError(err);
    }
...
server.registerTool(
  ...
  {
    description:
      "Submit code to Newton (runs hidden tests). problemId can be playground hash or assignment-question hash.",
    inputSchema: z.object({
      problemId: z.string(),
      language: z.string(),
      code: z.string(),
