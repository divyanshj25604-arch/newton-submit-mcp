...
import { getCurrentUser, fetchProblem, getSubmissionStatus, listAssignmentQuestions, submitSolution, } from "./newtonApi.js";
import { getEnvNewtonAuth, hasNewtonAuth, } from "./auth.js";
import { createSessionAuthStore } from "./sessionStore.js";
let sessionAuthStore;
let defaultAuth = loadPersistedAuth();
...
function readAuthFromCodexConfig() {
    try {
...
async function resolveAuth(extra) {
...
async function authForApi(extra) {
...
async function storeAuth(extra, auth, remember) {
    const normalized = normalizeAuthInput(auth);
    if (!hasNewtonAuth(normalized)) {
...
async function clearAuth(extra, clearSaved) {
    const sid = extra?.sessionId;
    if (sid && sessionAuthStore) {
...
function createServer() {
    ...
    server.registerTool("auth_login", {
        description: "Authenticate this MCP session. Pass sessionCookie/authToken, or set useCodexConfig=true to import from ~/.codex/config.toml.",
    ...
    }, async ({ sessionCookie, authToken, useCodexConfig, remember }, extra) => {
        try {
            ...
            if (!hasNewtonAuth(auth)) {
                throw new Error("No auth received. Provide sessionCookie/authToken, or set useCodexConfig=true.");
            }
    ...
    server.registerTool("auth_logout", {
        description: "Clear auth for this session. Set clearSaved=true to also remove persisted local auth file.",
        inputSchema: z.object({
            clearSaved: z.boolean().optional(),
        }),
    }, async ({ clearSaved }, extra) => {
    ...
    server.registerTool("get_problem", {
        description: "Fetch a Newton playground problem (coding/react/frontend/project-newton-box). problemId can be playground hash, assignment-question hash, or course/assignment/question reference.",
        inputSchema: z.object({
            problemId: z.string(),
            courseHash: z.string().optional(),
            assignmentHash: z.string().optional(),
        }),
    }, async ({ problemId, courseHash, assignmentHash }, extra) => {
        try {
            const auth = await authForApi(extra);
            return toolSuccess(await fetchProblem(problemId, {
                courseHash,
                assignmentHash,
                auth,
            }));
    ...
    server.registerTool("submit_solution", {
        description: "Submit code to Newton playgrounds (coding/react/frontend/project-newton-box). problemId can be playground hash or assignment-question hash.",
        inputSchema: z.object({
            problemId: z.string(),
            language: z.string(),
            code: z.string(),
    ...
    }, async ({ problemId, language, code, courseHash, assignmentHash, playgroundType }, extra) => {
        try {
            const auth = await authForApi(extra);
...
function enforceApiKeyIfNeeded(req, res) {
...
async function startHttpServer() {
    ...
    app.use((req, res, next) => {
        if (req.path === "/health") {
            if (!enforceApiKeyIfNeeded(req, res))
                return;
            next();
            return;
        }
